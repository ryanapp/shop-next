...
- `placedAt` is an ISO timestamp supplied by the server.
...
Rule loading must never use arbitrary paths from user input. Paths must be generated by the server and constrained to the generated directory.
...
7. Run `@openai/codex-sdk` server-side only.
...
15. Run `npx vitest run <generated-test-path>` with a timeout.
...
- Meaningful tests cover adapter, engine, loader, API, generation success, generation failure, policy rejection, and negative-discount safety.
- `npm test` passes.
